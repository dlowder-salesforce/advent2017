...
const buildGroup = function(nodemap, initialnode, set) {
  if (!set.contains(initialnode)) {
    set.add(initialnode);
  }
  var neighbors = nodemap[initialnode];
  for (var i in neighbors) {
    if (!set.contains(neighbors[i])) {
      buildGroup(nodemap, neighbors[i], set);
    }
  }
};

const findGroup = function(nodemap, initialnode) {
  var set = new treeset();
  buildGroup(nodemap, initialnode, set);
  return set;
};

const buildNodemap = function(input) {
  var nodemap = {};
  for (var i in input) {
    var tokens = input[i].split(' <-> ');
    var node = tokens[0].trim();
    var connected_nodes = tokens[1].split(', ');
    nodemap[node] = connected_nodes;
  }
  return nodemap;
};

const solution = function(input) {
  var nodemap = buildNodemap(input);
  var sets = [];
  var set = findGroup(nodemap, '0', set);
  sets.push(set);
...
const advent12 = function(callback) {
  io.readInputAsLines('./input12.txt', function(input) {
    var result = solution(input);
    let output = 'Day 12: ' + result.group0size + ' ' + result.ngroups;
