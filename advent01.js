...
const findIndexToComparePart1 = function(input, i) {
  return (i + 1) % input.length;
};

const findIndexToComparePart2 = function(input, i) {
  return (i + input.length / 2) % input.length;
};

const solution = function(input, findIndexFunction) {
  var sum = 0;
  for (var i = 0; i < input.length; i++) {
    var j = findIndexFunction(input, i);
    if (input[i] === input[j]) {
      sum += input[i];
    }
  }
...
const advent01 = function(callback) {
  io.readInputAsString('./input01.txt', function(input) {
    ...
    var output =
      'Day 1: ' +
      solution(inputDigits, findIndexToComparePart1) +
      ' ' +
      solution(inputDigits, findIndexToComparePart2);
