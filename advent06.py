...
def iterate(startblocks):
    blocks = [x for x in startblocks]

    maxvalue = max(blocks)
    imax = blocks.index(maxvalue)
    blocks[imax] = 0
    for j in range(maxvalue):
        k = (imax + j + 1) % len(blocks)
        blocks[k] += 1

    return blocks

input_file = open('./input06.txt','r')
initialBlocks = [int(x) for x in input_file.readline().strip().split('\t')]
input_file.close()
steps = []
