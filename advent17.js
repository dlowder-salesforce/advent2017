...
circular buffer some number of steps (your puzzle input) before inserting 
...
number of steps, and wherever it stops, inserts after it the second new 
...
the last value written (2017), you can short-circuit the spinlock. In this 
...
const advent17 = function(callback) {
  ...
  for (var i = 1; i <= 2017; i++) {
    ...
    buffer = buffer
      .slice(0, pos)
      .concat([i])
      .concat(buffer.slice(pos, buffer.length));
  }

  var solution1 = buffer[(pos + 1) % (i + 1)];

  // For part 2 we only care about changes in the first few elements of the buffer
  startelements = [0, 0];
  pos = 0;
  for (var i = 1; i <= 500000000; i++) {
    pos = (pos + stepsize + 1) % i;
    if (pos === 0) {
      startelements[1] = startelements[0];
      startelements[0] = i;
    }
    if (pos === 1) {
      startelements[1] = i;
  ...
  var solution2 = startelements[1];

  let output = 'Day 17: ' + solution1 + ' ' + solution2;
