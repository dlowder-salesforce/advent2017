...
def mutatefunction1(x):
    return x + 1

def mutatefunction2(x):
    return x - 1 if x >= 3 else x + 1

def solution(input, mutatefunction):
    loc = 0
    steps = 0
    length = len(input)
    while loc < length:
        newloc = loc + input[loc]
        input[loc] = mutatefunction(input[loc])
        loc = newloc
        steps += 1
    return steps
...
input = [int(l.strip()) for l in lines]
print(solution(input, mutatefunction1))
input = [int(l.strip()) for l in lines]
print(solution(input, mutatefunction2))
