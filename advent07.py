...
def treeweight(nodemap, key):
    node = nodemap[key]
    if node.get('treeweight', None) == None:
        node['treeweight'] = node['weight'] + sum([treeweight(nodemap, c) for c in node['childNodes']])
    return node['treeweight']

def buildTree(input):
    nodemap = {}
    node_with_children_rex = re.compile('^([a-z]+) \((\d+)\) -> (.+)')
    node_rex = re.compile('^([a-z]+) \((\d+)\)')
    for i in range(len(input)):
        ...
        if m:
            ...
            m = node_with_children_rex.match(s)
            if m:
    ...
    root = ''
    for key in nodemap:
        if nodemap.get(key).get('parentNode', None) == None:
            root = key
            break
    nodemap['root'] = root
    return nodemap

def uniqueChild(nodemap, key):
    children = [{
        'key': c,
        'weight': treeweight(nodemap, c)
    } for c in nodemap[key]['childNodes']]
    weightmap = {}
    for c in children:
        count = weightmap.get(str(c['weight']), 0)
        count += 1
        weightmap[str(c['weight'])] = count

    for c in children:
        if weightmap[str(c['weight'])] == 1:
            return c['key']
...
badchild = [c for c in children if c['key'] == uniqueChildKey][0]
goodchild = [c for c in children if c['key'] != uniqueChildKey][0]

# The weight we want is the weight needed to make all subtrees at this level
# have the same total weight
correctweight = badchild['weight'] + goodchild['totalweight'] - badchild['totalweight']

print(str(correctweight))
